...
.factory('userService', ['$http', function($http){
  const loginUri = 'http://localhost:8999/users/authenticate';
  const registerUri = 'http://localhost:8999/users/';
  const authRedirect = '//www.google.com';
  ...
  function register(username, password){
    return $http.post(registerUri, {username, password});
  }
...
.controller('RegisterController', ['$scope', 'userService', '$timeout', function($scope, userService, $timeout){
  ...
  $scope.register = function(){
    ...
    userService.register($scope.username, $scope.password).success(res=>{
      $scope.status = res;
      if (typeof(res) == 'string' && res.slice(0,5) == 'Error'){
        $timeout(()=>$scope.status='',2500);
      }
